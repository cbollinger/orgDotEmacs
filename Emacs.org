...
- Ubuntu and copy the resources into the desired locations
#+begin_src sh
mkdir .fonts
...
#+begin_src bash
  cd ~/Work/07-dg-git/
  git clone https://gitlab2.duagon.com/tools/docker/latex.git 
#+end_src

*** copy latex files
#+begin_src sh
  mkdir -p ~/texmf/tex/xelatex/duagon_public
  cd ~/Work/07-dg-git/latex/files/latex/texmf/tex/xelatex/duagon_public
  cp -r * ~/texmf/tex/xelatex/duagon_public
  mktexlsr ~/texmf
#+end_src

*** copy fonts
#+begin_src bash
  sudo mkdir -p /usr/share/fonts/truetype/duagon
  cd ~/Work/07-dg-git/latex/files/fonts
  cp * /usr/share/fonts/truetype/duagon
  sudo fc-cache -fv
#+end_src
...
sudo zypper install hunspell hunspell-tools
...
sudo zypper install texlive-collection-latex
sudo zypper install texlive-bclogo
sudo zypper install texlive-lastpage
sudo zypper install texlive-rotfloat
sudo zypper install texlive-mathtools
sudo zypper install texlive-tcolorbox
sudo zypper install texlive-comment
sudo zypper install texlive-ht-tikz
...
** Using your Hunspell + Flycheck setup
*** 1. Automatic spell checking
- Org/Text/LaTeX: Uses multi-dictionary (`de_CH,en_GB,en_US`) automatically.
- Programming modes: Uses English-only (`en_US`).
- Misspelled words are highlighted as you type.

*** 2. Manual spell checking
...
*** 3. Correcting words
- Highlighted word → correct using:
  M-x flycheck-correct-word
- Optional: with `flyspell-correct`:
  - Bind a key, e.g., C-; to `flyspell-correct-wrapper`
  - Shows an interactive popup menu for corrections

*** 4. Switching dictionaries on the fly
- Temporarily switch dictionaries:
  M-x ispell-change-dictionary
- Example: switch from `multi-de-en` to `en_US`

*** 5. Adding personal words
- Add frequently accepted words:
  M-x ispell-add-word-to-personal-dictionary
- Stored in `~/.hunspell_personal`
- Automatically recognized in future sessions

*** 6. Debugging / info
- List dictionaries Hunspell knows:
  shell> hunspell -D
- Check current dictionary in Emacs:
  C-h v ispell-dictionary
- Run `M-x ispell-buffer` to see spell check in action

*** 7. Tips
- Multi-dictionary only works with plain Flyspell / Flycheck (phaf disabled)
- Programming modes remain English-only
- Keep your `.hunspell_personal` up to date for new words
- Consider keybindings for quick corrections and dictionary switching
...
Set Up Hunspell dictionnaries and set hunspell as backend
#+begin_src emacs-lisp
;; --- Hunspell setup ---
...
Set Flycheck and flycheck-aspell as frontend
#+begin_src emacs-lisp
...
** Org Custom Agenda View                         :business_view:
...
** COMMENT Org Custom Agenda View                                       :private_view:
...
for gonme use: org-attach-screenshot-command-line "gnome-screenshot -a -f %f")
#+begin_src emacs-lisp
...
Global latexmk settings, tangles to =~/.latexmkrc=.
The shell-escape flag is required by =minted= for syntax highlighting.
=max_repeat= is capped at 3 (default is 5) — enough for =hyperref= + =lastpage= + =scrlayer-scrpage= to converge, while avoiding slow extra passes on documents with many code blocks.

#+begin_src conf :tangle ~/.latexmkrc :mkdirp yes
# Global latexmk configuration — managed via Emacs.org

# Include -shell-escape so minted can call Pygments; halt on error for fast feedback
...
# Limit reruns to 3 (default is 5).
# With lastpage+scrlayer-scrpage+hyperref, 2-3 passes are always enough.
$max_repeat = 3;

# Remove intermediate xdv and synctex files on latexmk -c
$clean_ext = 'xdv synctex.gz';

# Upgrade PDF output to 1.7 so included PDFs (logos, images) don't trigger version mismatch warnings
$xdvipdfmx = 'xdvipdfmx -V 7 %O -o %D %S';
#+end_src

** Org Export Latex Headlines
Colored Format for TODO/DONE Headlines
...
(defun my-tab-complete ()
  "Expand a Yasnippet, accept Company completion, or indent."
  (interactive)
  (cond
   ((and (yas-minor-mode)
...
(with-eval-after-load 'company
  (define-key company-active-map (kbd "TAB") #'my-tab-complete)
  (define-key company-active-map (kbd "<tab>") #'my-tab-complete))

(with-eval-after-load 'yasnippet
  (define-key yas-minor-mode-map (kbd "TAB") #'my-tab-complete)
  (define-key yas-minor-mode-map (kbd "<tab>") #'my-tab-complete))
#+end_src
...
[[https://www.flycheck.org/en/latest/][Flycheck]] Syntax checking for GNU Emacs — configured in the Spell Checking section above.
