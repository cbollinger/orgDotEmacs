...
Also sets the command =C-x X-b= to ibuffer!
...
(custom-set-faces
 '(org-column ((t (:height 140 :background "gray30")))))
...
#+begin_src emacs-lisp
  ...
  (use-package indium
    :ensure t
    :init
    (setq indium-verbosity "debug")  ;; Set verbosity to debug for detailed logging
    :hook (js2-mode . indium-interaction-mode)
    :config
    ;; Function to start Indium client

    (defun my-start-indium-client ()
      "Start the Indium client and send a request to list configurations."
      (interactive)
      (unless (indium-client-process-live-p)
        (indium-start-process)) ;; Start the Indium process if not already started

      ;; Wait for the process to be fully initialized
      (while (not (indium-client-process-live-p))
        (sleep-for 0.1))

      (let* ((request `((type . "configurations")
                        (payload (action . "list")
                                 (directory . ,(projectile-project-root)))))
             (callback (lambda (configurations)
                         (message "Configurations: %s" configurations))))
        (indium-client-send request callback)))
    ;; Keybinding to start Indium client
    (global-set-key (kbd "C-c i") 'my-start-indium-client))
